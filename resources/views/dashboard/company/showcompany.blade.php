        ...
        <div class="max-w-4xl mx-auto">
            <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="mt-1 text-gray-900">
                                @if(!empty($company->company_website))
                                    <a href="{{ $company->company_website }}" class="text-blue-600 hover:underline" target="_blank">{{ $company->company_website }}</a>
                                @else
                                    -
                                @endif
                            </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="mt-1">
                                @if(!empty($company->company_logo))
                                    <img src="{{ asset('storage/' . $company->company_logo) }}" alt="Logo {{ $company->company_name }}" class="h-24 rounded-md shadow border">
                                @else
                    ...
                    <form action="{{ route('company.destroy', $company->uid) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data perusahaan ini?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
